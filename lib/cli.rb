class Cli 
  ...
  def navigation(arg)
    if arg == "Y" || arg == "y"
      ...
      new_stock = Stock.create(stock: Stock.stock_generator[0], ticker_symbol: Stock.stock_generator[1], price: Stock.stock_generator[2])
      Portfolio.create(user_id: new_user.id, stock_id: new_stock.id, company: user_brokerage, account_number: random_account_generator, account_value: Stock.stock_generator[2])
      puts "Congrats #{user_name}! Your new account number is #{random_account_generator}. You have also been granted a free stock of #{Stock.stock_generator[0]} at a value of #{Stock.stock_generator[2]}!"
      options(user_name)
    else 
  ...
  def choices(arg)
    if arg == 1 
      ...
      puts (Portfolio.find_by(account_number: account_num).account_value)
      return_to_menu
    elsif arg == 2
      puts "Enter your account number"
      account_num = gets.strip.to_i
